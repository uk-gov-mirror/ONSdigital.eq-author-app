...
const convertQuestionniare = async (questionnaire, publishResult) => {
  ...
  if (convertedResponse.status !== 200) {
    ...
    throw new Error(
      `Publisher failed to convert questionnaire - ${convertedResponse.statusText}`
    );
  }
  ...
  await authorisedRequest(
    ...
    .then(async (res) => {
      if (res.status === 200) {
        ...
        publishResult.displayErrorMessage = "Contact eQ services team";
        throw new Error(
          `Invalid response - failed with error code ${res.status}`
        );
      }
    })
    .catch((e) => {
...
const publishSchema = async (ctx) => {
  ...
  try {
    ...
    if (
      process.env.CIR_PUBLISH_SCHEMA_GATEWAY_SECOND !== "nopublish" &&
...
const republishSchema = async (questionnaireVersionId, cirVersion) => {
  ...
  try {
    ...
    logger.info(
      publishResult,
      `republishing questionnaire with version id ${questionnaire.questionnaireVersionId}  - posted to first CIR gateway`
    );
    ...
    logger.info(
      publishResult,
      `republishing questionnaire with version id ${questionnaire.questionnaireVersionId}  - posted to second CIR gateway`
    );
  } finally {
    if (!publishResult.success) {
      logger.error(publishResult.errorMessage, "Republish failed");
